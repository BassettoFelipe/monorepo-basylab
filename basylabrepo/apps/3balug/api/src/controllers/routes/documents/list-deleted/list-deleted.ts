...
import { listDeletedDocumentsParamsSchema, listDeletedDocumentsResponseSchema } from './schema'

const SIGNED_URL_EXPIRY = 3600

export const listDeletedDocumentsController = new Elysia().guard({ as: 'local' }, (app) =>
	app
		.use(requireAuth)
		.use(validateUserState)
		.use(requireRole([USER_ROLES.OWNER, USER_ROLES.MANAGER, USER_ROLES.BROKER]))
		.get(
			'/documents/:entityType/:entityId/deleted',
			async ({ validatedUser, params }) => {
				const result = await documentsContainer.listDeleted.execute({
			...
			{
				params: listDeletedDocumentsParamsSchema,
				response: {
					200: listDeletedDocumentsResponseSchema,
				},
			},
		),
)
