...
import { getPropertyParamsSchema, getPropertyResponseSchema } from "./schema";

export const getPropertyController = new Elysia().guard({ as: "local" }, (app) =>
  app
    .use(requireAuth)
    ...
    .get(
      ...
      async ({ validatedUser, params }) => {
        const result = await container.properties.get.execute({
          id: params.id,
