...
export const activateUserController = new Elysia().guard({ as: 'local' }, (app) =>
	app
		.use(requireAuth)
		...
		.patch(
			...
			async ({ validatedUser, params }) => {
				const result = await container.users.activateUser.execute({
					userId: params.id,
					activatedBy: validatedUser,
				})
				...
				logger.info(
					{
						...
						userId: result.id,
						email: result.email,
						activatedBy: validatedUser.id,
					},
					`Usuário ativado: ${result.email}`,
				)
