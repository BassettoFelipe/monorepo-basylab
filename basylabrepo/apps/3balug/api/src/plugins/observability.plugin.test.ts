...
import * as metricsModule from '@/plugins/metrics.plugin'
import { observabilityPlugin } from './observability.plugin'

// Use spyOn instead of mock.module to avoid global module pollution
const mockRecordHttpMetrics = spyOn(metricsModule, 'recordHttpMetrics').mockImplementation(() => {})

afterAll(() => {
	mockRecordHttpMetrics.mockRestore()
})
...
describe('observabilityPlugin', () => {
	...
	beforeEach(() => {
		...
		mockRecordHttpMetrics.mockClear()
	})
	...
	describe('metrics', () => {
		it('should not record metrics for /metrics path', async () => {
			...
			expect(mockRecordHttpMetrics).not.toHaveBeenCalled()
		})
		...
		it('should record metrics for regular paths', async () => {
			...
			expect(mockRecordHttpMetrics).toHaveBeenCalled()
		})
