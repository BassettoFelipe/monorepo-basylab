...
const mockValidatePasswordStrength = spyOn(
	Validators,
	'validatePasswordStrength',
).mockImplementation((password: string): string[] => {
	...
	describe('Email failures', () => {
		test('rolls back on email failure', async () => {
			...
			mockSendVerificationCode.mockImplementation(() =>
				Promise.reject(new EmailServiceError('SMTP down', new Error('Connection failed'))),
			)
