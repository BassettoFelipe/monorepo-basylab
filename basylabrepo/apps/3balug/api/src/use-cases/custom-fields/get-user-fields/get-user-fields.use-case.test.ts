...
describe('GetUserFieldsUseCase', () => {
	...
	describe('Casos de Sucesso', () => {
		...
		test('deve buscar campos do usuário alvo correto', async () => {
			...
			expect(mockCustomFieldResponseRepository.findByUserId).toHaveBeenCalledWith('target-user-123')
		})
	...
	describe('Mensagens de Erro', () => {
		...
		test('deve retornar mensagem específica quando target está em outra empresa', async () => {
			...
			try {
				...
				expect((error as Error).message).toBe('Você não tem permissão para visualizar este usuário')
			}
		})
	})
