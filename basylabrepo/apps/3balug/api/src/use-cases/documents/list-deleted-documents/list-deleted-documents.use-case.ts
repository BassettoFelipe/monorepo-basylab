...
interface ListDeletedDocumentsInput {
...
interface DeletedDocumentOutput {
	id: string
	entityType: string
	entityId: string
	documentType: string
...
interface ListDeletedDocumentsOutput {
	documents: DeletedDocumentOutput[]
	total: number
}

const ALLOWED_ROLES: UserRole[] = [USER_ROLES.OWNER, USER_ROLES.MANAGER, USER_ROLES.BROKER]

export class ListDeletedDocumentsUseCase {
	constructor(
	...
	async execute(input: ListDeletedDocumentsInput): Promise<ListDeletedDocumentsOutput> {
		const { entityType, entityId, user } = input

		if (!ALLOWED_ROLES.includes(user.role as UserRole)) {
			throw new ForbiddenError('Voce nao tem permissao para listar documentos excluidos.')
		}
		...
		try {
			...
			const [documents, total] = await Promise.all([
				this.documentRepository.findDeletedByEntity(entityType, entityId, {
					limit,
					offset,
				}),
				this.documentRepository.countDeletedByEntity(entityType, entityId),
			])
			...
			logger.error({ err: error, entityType, entityId }, 'Erro ao listar documentos excluidos')
			throw new InternalServerError('Erro ao listar documentos excluidos. Tente novamente.')
		}
	}
}
