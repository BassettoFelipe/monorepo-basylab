...
describe('UpdateTenantUseCase', () => {
	...
	beforeEach(async () => {
		...
		// Create use case
		useCase = new UpdateTenantUseCase(tenantRepository, documentValidator, contactValidator)
	...
	describe('Validações de Erro', () => {
		...
		test('deve lançar erro se locatário não existe', async () => {
			...
			).rejects.toThrow('Locatário não encontrado')
		})
	...
	describe('Controle de Permissões - BROKER', () => {
		...
		test('BROKER não deve poder editar locatário criado por outro usuário', async () => {
			...
			).rejects.toThrow('Você só pode editar locatários que você cadastrou')
		})
