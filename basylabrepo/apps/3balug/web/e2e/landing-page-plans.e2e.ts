...
test.describe("Landing Page - Plans Section", () => {
	...
	test("deve validar estrutura de dados de cada plano", async ({ page }) => {
		...
		for (let i = 0; i < planCount; i++) {
			...
			console.log(
				`✓ Plano ${i + 1}: ${nameText} - ${priceText} - ${featureCount} features`,
			);
		}
	});

	test("deve validar dados específicos esperados dos planos", async ({
		page,
	}) => {
		// Aguardar os planos carregarem
		await page.waitForSelector('[data-testid="plan-card"]', {
		...
		for (let i = 0; i < planCount; i++) {
			...
			const name =
				(await card.locator('[data-testid="plan-name"]').textContent()) || "";
			const price =
				(await card.locator('[data-testid="plan-price"]').textContent()) || "";
		...
		for (const plan of plansData) {
			...
			const hasSerasaInfo = plan.features.some(
				(f) =>
					f.toLowerCase().includes("serasa") ||
					f.toLowerCase().includes("consulta"),
			);
			...
			const hasPropertyManagement = plan.features.some(
				(f) =>
					f.toLowerCase().includes("imóveis") ||
					f.toLowerCase().includes("imoveis"),
			);
			expect(hasPropertyManagement).toBe(true);

			// Todos os planos devem ter contratos digitais
			const hasDigitalContracts = plan.features.some((f) =>
				f.toLowerCase().includes("contrato"),
			);
			expect(hasDigitalContracts).toBe(true);

			console.log(
				`✓ Plano "${plan.name}" possui todas as features obrigatórias`,
			);
		}
	});
	...
	test("deve validar que a API retorna dados válidos", async ({ page }) => {
		...
		const apiResponse = await page.waitForResponse(
			(response) =>
				response.url().includes("/plans") && response.status() === 200,
			{ timeout: 10000 },
		);
	...
	test("deve mostrar skeleton loader enquanto carrega", async ({ page }) => {
		...
		const skeletonCount = await page
			.locator('[data-testid="plan-skeleton"]')
			.count();
		if (skeletonCount > 0) {
