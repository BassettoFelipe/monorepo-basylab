...
import { type CreateTenantFormData, createTenantSchema } from '@/schemas/tenant.schema'
...
import { MARITAL_STATUS_LABELS } from '@/types/tenant.types'
...
import * as styles from './CreateTenantModal.styles.css'
...
const INITIAL_DOCUMENT_SLOTS: DocumentSlot[] = [
	...
	{ type: DOCUMENT_TYPES.COMPROVANTE_RENDA, label: 'Comprovante de Renda', files: [] },
...
export function CreateTenantModal({ isOpen, onClose }: CreateTenantModalProps) {
	...
	const documentSlotsRef = useRef<DocumentSlot[]>([])
	...
	useEffect(() => {
		documentSlotsRef.current = documentSlots
	}, [documentSlots])

	useEffect(() => {
		return () => {
			for (const slot of documentSlotsRef.current) {
				for (const file of slot.files) {
					if (file.preview) {
						URL.revokeObjectURL(file.preview)
					}
	...
	const onSubmit = async (data: CreateTenantFormData) => {
		...
		try {
			...
			const payload = {
				...
				rg: data.rg?.replace(/\D/g, '') || undefined,
			...
			// Primeiro cria o inquilino sem a foto
			...
			if (photoFile) {
				...
				try {
					...
					// Atualiza o inquilino com a URL da foto
					...
					toast.error(`Inquilino criado, mas houve erro ao enviar foto: ${errorMsg}`)
				} finally {
			...
			if (slotsWithFiles.length > 0) {
				...
				try {
					...
					toast.error(`Inquilino criado, mas houve erro ao enviar documentos: ${errorMsg}`)
				} finally {
			...
			const errorMessage = handleApiError(error, 'Erro ao criar inquilino')
		...
		<div className={styles.customHeader}>
			...
			<div className={styles.headerLeft}>
				<div className={styles.headerInfo}>
					<h2 className={styles.headerTitle}>Adicionar Inquilino</h2>
				...
				{currentStep === 5 && (
					<div className={styles.formSection}>
						<div className={styles.documentsInfo}>
							<Info size={18} className={styles.documentsInfoIcon} />
							<p className={styles.documentsInfoText}>
								Anexe os documentos do inquilino. Todos os campos sao opcionais. Formatos aceitos:
								PDF, JPG, PNG ou WebP. Voce pode enviar multiplos arquivos por tipo (frente, verso,
								etc).
							</p>
						</div>
